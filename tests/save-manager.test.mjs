...
const envelope=createEnvelope(normalized,{appVersion:'v0.13.0',schemaVersion:5});
assert.equal(verifyEnvelope(envelope,{schemaVersion:5}).ok,true);
envelope.payload.player.xp=999;
assert.equal(verifyEnvelope(envelope,{schemaVersion:5}).ok,false,'Checksum deveria detectar alteração.');
const storage=new MemoryStorage();
const manager=new SafeSaveManager({appVersion:'v0.13.0',schemaVersion:5,context,storage});
...
const legacyManager=new SafeSaveManager({appVersion:'v0.13.0',schemaVersion:5,context,storage:legacyStorage});
...
console.log('PASS save-manager: schema v5, triagem, ramificações, academia, checksum, backup, recuperação e migração.');
