window.C190_Multitask = (() => {
  ...
  function updateShift(state, shift = state?.dispatch?.shift) {
    ...
    waiting.forEach((call) => {
      ...
      risk.score = Math.max(0, Math.min(100, Number(risk.score || 0) + Number(urbanCall.riskBonus || 0) + Number(majorCall.riskBonus || 0) + Number(supportCall.riskBonus || 0) + Number(commandCall.riskBonus || 0) + Number(fatigueCall.riskBonus || 0)));
      if ((urbanCall.riskBonus > 0 || majorCall.riskBonus > 0 || supportCall.riskBonus > 0 || commandCall.riskBonus > 0 || fatigueCall.riskBonus > 0) && risk.level !== "critical") risk.level = risk.score >= 86 ? "critical" : risk.score >= 58 ? "high" : risk.level;
